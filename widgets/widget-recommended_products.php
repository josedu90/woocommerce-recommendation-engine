...
class WooCommerce_Widget_Recommended_Products extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        $activity_type = isset($instance['activity']) ? $instance['activity'] : array('viewed', 'completed');
        ?>
    ...
    function update($new_instance, $old_instance) {
        ...
        $instance['activity'] = isset($new_instance['activity']) ? $new_instance['activity'] : array('viewed', 'completed');
    ...
    function form($instance) {
        ...
        <p>
        <p><label for="<?php echo $this->get_field_id('activity'); ?>"><?php _e('Show recommendations based on:', 'woocommerce'); ?></label>
            <?php
            $activities = array('viewed' => 'View History', 'completed' => 'Purchase History');
            $options = '<ul>';
            foreach ($activities as $activity => $label) {
                $options .= '<li><label class="selectit">';
                $options .= '<input class="selectit" type="checkbox" id="' . $this->get_field_id('activity') . '[]" name="' . $this->get_field_name('activity') . '[]"';
                if (isset($instance['activity']) && is_array($instance['activity'])) {
                    foreach ($instance['activity'] as $current_activity) {
